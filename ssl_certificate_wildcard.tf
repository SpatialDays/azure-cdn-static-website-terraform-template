...
resource "acme_certificate" "root-wildcard-certificate" {
  ...
  depends_on = [acme_certificate.root-domain-certificate, time_sleep.sleep-10-minutes-after-making-dns-zone]
}

# create a random 8 char lowercase string
resource "random_string" "root-wildcard-random-string" {
  length     = 10
  special    = false
  upper      = false
  depends_on = [acme_certificate.root-wildcard-certificate]
}

resource "azurerm_key_vault_certificate" "root-wildcard-certificate" {
  name         = "root-wildcard-cert-kv-${random_string.root-wildcard-random-string.result}"
  key_vault_id = azurerm_key_vault.key_vault.id
  certificate {
    contents = acme_certificate.root-wildcard-certificate.certificate_p12
    password = ""
  }
  certificate_policy {
    issuer_parameters {
